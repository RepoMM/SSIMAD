...
<div class="students index large-10 medium-9 columns">
    ...
    <?php foreach ($students as $student): ?>
        <tr>
            ...
            <td><?= h($student->names) ?></td>
            <td><?= h($student->paternal_surname) ?></td>
            <td><?= h($student->maternal_surname) ?></td>
            <td class="actions">
                <?= $this->Html->link(__('View'), ['action' => 'view', $student->id]) ?>
                <?= $this->Html->link(__('Edit'), ['action' => 'edit', $student->id]) ?>
                <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $student->id], ['confirm' => __('Are you sure you want to delete # {0}?', $student->username)]) ?>
            </td>
        </tr>
